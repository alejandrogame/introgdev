...
audio_sound_gain(snd_house, 0, 0);
function play_music() {
	
	if (place_meeting(x,y,obj_house_zone)) {
		
		if (music != snd_house) {
			
			if (music != noone) audio_sound_gain(music, 0, fade_time);
			
			music = snd_house;
			if (!audio_is_playing(music)) audio_play_sound(music,0,1);
			audio_sound_gain(music, .5, fade_time);
			
		}
		
	}
	
	else if (music != snd_background) {
		
		if (music != noone) audio_sound_gain(music, 0, fade_time);
		
		music = snd_background;
...
function spawn_textbox() {
	...
	if (x > textbox.x + textbox.sprite_width/2 + far_away_from_text || 
		...
		y > textbox.y + camera_get_view_height(view_camera[0])/6 + textbox.sprite_height/3 + far_away_from_text || 
		y < textbox.y + camera_get_view_height(view_camera[0])/6 + textbox.sprite_height/3 - far_away_from_text) {
		instance_destroy(textbox);
		textbox = noone;
	}
...
function knock() {
	...
	if (keyboard_check_pressed(vk_space) && place_meeting(x,y-vel,obj_door)) audio_play_sound(snd_knock,2,0);
